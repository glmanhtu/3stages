...
import numpy as np
import torch
from facenet_pytorch.models.utils.detect_face import batched_nms_numpy, bbreg, imresample, pad, rerec, \
    generateBoundingBox
...
def extract_face(img, box, image_size=160, margin=0):
    """Extract face + margin from np Image given bounding box.

    Arguments:
        img {numpy.ndarray} -- A Numpy Image.
    ...
    box = [
        ...
        int(min(box[2] + margin[0] / 2, img.shape[1])),
        int(min(box[3] + margin[1] / 2, img.shape[0])),
    ]
    width = abs(box[0] - box[2])
    height = abs(box[1] - box[3])
    if width < height:
        different = height - width
        box[0] -= int(different / 2)
        box[2] += int(different / 2)
    elif width > height:
        different = width - height
        box[1] -= int(different / 2)
        box[3] += int(different / 2)
